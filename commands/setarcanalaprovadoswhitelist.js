    ...
    Database.BemVindo .findOne({guilda: message.guild.id}, function(erro, dados) {
        if(!dados){
            let embed = new Discord.MessageEmbed()
                ...
                .setColor("RANDOM") 
                .setDescription(`Canal de aprovados de whitelist setado com sucesso para ${canal}`)
            ...
            let embed = new Discord.MessageEmbed()
                ...
                .setColor("RANDOM") 
                .setDescription(`**Canal de aprovados de Whitelist setado com sucesso! <a:verify:687759351330766901> \n \n  O canal setada como: **\n${canal}`)
                .setFooter(message.guild.name + " © 2020", client.user.avatarURL)
                .setTimestamp();
                message.channel.send(embed).then(msg => msg.delete({ timeout: 5000, reason: 'It had to be done.' }));
                dados.aprovado = canal.id
                dados.save();
        }
        })
