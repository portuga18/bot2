...
exports.run = async(client, message, channel) => {

    message.delete()

if(!message.member.hasPermission("SEND_MESSAGES")) return message.reply("você não tem a permissão ``SEND_MESSAGES``!");
message.guild.channels.create(`Denuncia`, {reason: 'New channel added for fun!', type: 'text'}).then(canal => {
    canal.permissionOverwrites(message.guild.defaultRole, {
        VIEW_CHANNEL: false
    })
    canal.permissionOverwrites(message.member, {
        VIEW_CHANNEL: true
    })
...
canal.send("Você deseja enviar esta Report? Se sim digite ``enviar`` Caso contrário, digite ``cancelar``!")
    ...
    console.log("Ao enviar denuncia")
}
